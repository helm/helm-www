...
To retrieve logs from test pods after tests complete, use the `--logs` flag:

```console
$ helm test demo --logs
```

If test pods have multiple containers (e.g., service mesh sidecars like Istio, Consul, or Vault), logs are fetched from all containers, including init containers. The output labels each container:

```
POD LOGS: demo-test-connection (wget)
...
