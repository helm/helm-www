...
Use the `--output` flag to get push results in a structured format. Available formats are `table` (default), `json`, and `yaml`.
...
This is useful for CI/CD pipelines and integration with signing tools like [cosign](https://github.com/sigstore/cosign). For example, you can capture the digest to sign the pushed artifact:
