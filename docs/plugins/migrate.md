...
Your existing Helm 3 plugins still work in Helm 4, including plugins
that use the `subprocess` runtime. What is legacy is the
non-versioned Helm 3 `plugin.yaml` schema; this guide walks you
through updating a Helm 3 plugin to the versioned
...
[Helm 4 Overview](/overview.md#plugin-system-overhaul) for details.
:::
...
The key changes are: add `apiVersion`, `type`, and `runtime` at
the top level; optionally add `sourceURL` there as well for
transparency; nest `usage`, `description` (now `shortHelp`), and
`ignoreFlags` under `config`; and nest `platformCommand` and
`platformHooks` under `runtimeConfig`.
...
scratch, see the
[Getter Plugin Wasm tutorial](/plugins/developer/tutorial-getter-plugin.mdx)
...
progress. Check the
[Developing Plugins](/plugins/developer/index.mdx) page for updates.
:::
...
- No `platformCommand` or `platformHooks` — Helm loads the Wasm
  binary from `plugin.wasm` in the plugin directory.
- The plugin directory contains `plugin.wasm` instead of
...
For plugin tarballs, verification is enabled by default. See
[Plugin Security](/plugins/user/index.md#plugin-security) for details.
...
- [Plugins Overview](/plugins/overview.md) — full `plugin.yaml`
  specification
- [Developing Plugins](/plugins/developer/index.mdx) — tutorials for
