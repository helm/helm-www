...
У цьому посібнику пояснюється, як налаштувати середовище для розробки з Helm.
...
Єдине виключення: [вивід CLI Helm (англійською)](/helm/index.mdx) генеруються безпосередньо з бінарного файлу `helm`. Дивіться [Оновлення довідкових документів CLI Helm](https://github.com/helm/helm-www#updating-the-helm-cli-reference-docs) для інструкцій, як згенерувати цей вивід. Після перекладу, вивід CLI не генерується і може бути знайдений у `i18n/uk/docusaurus-plugin-content-docs/current/docs/helm`.

### Домовленості Git {#git-conventions}

Ми використовуємо Git як систему контролю версій. Гілка `main` є основною для поточної версії, що перебуває на стадії розробки. Випуски позначаються теґами.
...
4. Коли ви будете готові до рецензцування коду, залийте вашу гілку на GitHub, а потім відкрийте новий pull request у нас.
...
Ми дуже ретельно дотримуємося стандартів стилю кодування Go. Зазвичай, запуск `go fmt` зробить ваш код охайнішми.
