...
헬름4부터 추가적인 플러그인 유형을 더 쉽게 추가할 수 있도록 플러그인 시스템이 설계되었고, 사용자는 헬름 기능의 다른 영억도 수정할 수 있게 됩니다.
...
만약 `type` 필드가 `postrenderer/v1`이라면 [포스트 렌더러 플러그인 유형](#cli-plugins)이며, 별도의 설정 옵션이 없습니다.
