---
title: Używanie Helma
description: Podstawy korzystania z Helma.
sidebar_position: 3
---
...
Ten przewodnik wyjaśnia podstawy używania Helma do zarządzania pakietami na Twoim
klastrze Kubernetesa. Zakłada, że masz już [zainstalowanego](/intro/install.mdx) klienta Helma.

Jeśli jesteś zainteresowany uruchomieniem kilku szybkich poleceń, możesz
zacząć od [przewodnika szybkiego startu](/intro/quickstart.md). Ten
rozdział obejmuje szczegóły dotyczące poleceń Helma i wyjaśnia, jak z niego korzystać.

## Trzy główne koncepcje {#three-big-concepts}

*Chart* to pakiet Helma. Zawiera wszystkie definicje
zasobów niezbędne do uruchomienia aplikacji, narzędzia lub
usługi w klastrze Kubernetesa. Można go porównać do
...
[bazy danych pakietów Fedora](https://src.fedoraproject.org/), ale przeznaczone dla pakietów Kubernetesa.

*Wydanie* (ang. Release) to instancja _charta_ działającego w klastrze Kubernetesa. Jeden
_chart_ może być często zainstalowany wiele razy w tym samym klastrze. Za każdym razem, gdy jest zainstalowany,
tworzone jest nowe _wydanie_. Weźmy pod uwagę _chart_ MySQL. Jeśli chcesz
mieć dwie bazy danych działające w twoim klastrze, możesz zainstalować ten _chart_ dwa razy.
...
Helm instaluje _chartsy_ w Kubernetesie, tworząc nową _wydanie_ dla każdej
instalacji. Aby znaleźć nowe charty, możesz przeszukiwać _repozytoria_ chartów.
...
- `helm search hub` przeszukuje [Artifact Hub](https://artifacthub.io),
  który zawiera listę chartów Helma z wielu różnych repozytoriów.
...
Polecenie `helm search hub` pokazuje URL do lokalizacji na [artifacthub.io](https://artifacthub.io/), ale nie do faktycznego repozytorium Helma.
`helm search hub --list-repo-url` pokazuje faktyczny URL repozytorium Helma, co jest przydatne, gdy chcesz dodać nowe repozytorium: `helm repo add [NAME] [URL]`.
...
Wyszukiwanie wykorzystuje algorytm dopasowania
...
temat tego, jakie zasoby zostały utworzone, jaki jest stan wydania, a także czy
...
Wiele chartów wymaga obrazów Dockera o rozmiarze
...
Sposób instalacji przedstawiony powyżej korzysta
jedynie z domyślnej konfiguracji chartu. Często jednak
zachodzi potrzeba dostosowania chartu do własnych ustawień.
...
Następnie możesz nadpisać dowolne z tych ustawień w
pliku w formacie YAML i przekazać ten plik podczas instalacji.
...
W ten sposób można ustawić wiele wartości. Linia
...
Głębokie zagnieżdżone struktury danych mogą być trudne do wyrażenia za pomocą `--set`. Projektanci
chartów powinni uwzględniać sposób użycia `--set` podczas projektowania formatu pliku `values.yaml`
(więcej informacji na temat [plików wartości (ang. Values Files)](/chart_template_guide/values_files.mdx)).

### Inne metody instalacji {#more-installation-methods}
...
dostarczonymi informacjami. Ponieważ charty Kubernetesa mogą być obszerne i
...
określić zachowanie Helma podczas instalacji/aktualizacji/przywracania.
Należy pamiętać, że nie jest to pełna lista flag CLI. Aby
...
- `--timeout`: Wartość typu [Go duration](https://golang.org/pkg/time/#ParseDuration),
  która określa, jak długo czekać na zakończenie poleceń Kubernetesa. Domyślnie jest to `5m0s`.
...
W poprzednich wersjach Helma, kiedy wydanie było usuwane,
pozostawał zapis jego usunięcia. W Helm 3, usunięcie usuwa również zapis o
...
upewnić się, że Twój klient Helma jest aktualny, uruchamiając `helm repo update`.

Repozytoria można usunąć za pomocą `helm repo remove`.

## Tworzenie własnych chartów {#creating-your-own-charts}

Przewodnik [tworzenia chartów](/topics/charts.mdx) wyjaśnia,
jak rozwijać własne charty. Możesz
jednak szybko rozpocząć, używając polecenia `helm create`:
...
Zpakowane charty mogą być ładowane do
repozytoriów. Zobacz dokumentację [repozytoriów chartów Helma](/topics/chart_repository.md)
...
poleceń, zapoznaj się z wbudowaną pomocą Helma: `helm help`.

W [następnym rozdziale](/howto/charts_tips_and_tricks.md) przyjrzymy się procesowi tworzenia chartów.
