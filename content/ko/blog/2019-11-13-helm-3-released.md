...
헬름 팀은 헬름 3의 첫번째 안정적인 릴리스를 발표하게 된 것을 자랑스럽게 생각한다.

헬름 3은 CLI 툴의 최신 주요 릴리스이다. 헬름 3은 헬름 2의 성공을 기반으로 진화하는 생태계의 요구를 지속적으로 충족한다.

헬름 3의 내부 구현은 헬름 2에서 상당히 변경되었다. 가장 명백한 변경은 틸러(Tiller)가 제거된 것이다. 그러나 새 릴리스에서 다른 변경 사항을 확인하는게 좋다.
커뮤니티의 의견과 요구사항에 따라 다양한 새로운 기능들이 추가되었다. 일부 기능은 헬름 2와 호환되지 않는 방식으로 더 이상 사용되지 않거나 리팩토링 되었다.
OCI 지원을 포함하는 일부 새로운 실험적 기능도 도입되었다.

또한, 헬름 Go SDK 는 일반적인 용도로 리팩토링 되었다.
목표는 더 광범위한 Go 커뮤니티와 오픈 소스로 만든 코드를 공유하고 재사용 하는 것이다.
헬름을 자체 프로젝트에 통합하는 다른 엔지니어의 의견을 적극적으로 찾고 있으며, [#헬름-개발 쿠버네티스 슬랙 채널](https://slack.k8s.io/)에서 여러분들의 의견을 듣고 싶다.
...
이 시점에서 스스로에게 물어볼 수 있다:

> 헬름 2에서 워크플로가 어떻게 변경되었나? 헬름 2로 이러한 명령을 실행하면 동일한 출력 결과가 표시될까?
