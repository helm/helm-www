...
2. [주/부 릴리스: Git에서 버전 번호 변경](#2-주부-릴리스-Git에서-버전-번호-변경)
...
[주/부 릴리스 : 
Git에서 버전 번호 변경] (#2-주부-릴리스-Git에서-버전-번호-변경).
...
최종적으로 릴리스 후보의 품질에 만족하면 계속 진행하여 실체를 
...
릴리스 정보를 살펴 보게 할 필요가 있다. 검토를 위해 
[#helm-dev](https://kubernetes.slack.com/messages/C51E88VDG)로 
요청을 보낸다. 우리 모두는 그 누구라도 무언가를 놓칠 수 있기 때문에 항상 도움이 된다.
