...
*차트*는 헬름 패키지이다. 
쿠버네티스 클러스터 내에서 애플리케이션, 도구, 서비스를 구동하기 위해 필요한 모든 리소스 정의들을 담고 있다.
...
- `helm search hub`는, 수십 가지의 여러 저장소들에 있는 헬름 차트들을 포괄하는, [헬름 허브](https://hub.helm.sh)를 검색한다.
- `helm search repo`는, 로컬 헬름 클라이언트에 (`helm repo add`로) 추가된 저장소들을 검색한다.
...
가장 간단하게는, 2개의 인수를 받는다:
사용자가 지정한 릴리스 이름, 설치하기 원하는 차트의 이름
...
여기서는 이 차트의 기본 설정옵션들만 사용할 것이다.
많은 경우에 선호하는 설정을 사용하기 위해 차트를 커스터마이징하게 될 것이다.
