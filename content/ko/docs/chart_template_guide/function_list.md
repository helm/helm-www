...
첫 번째 인수가 두 번째 인수보다 작 으면 부울 true를 반환한다. 
...
첫 번째 인수가 두 번째 인수보다 작거나 같은 경우 부울 true를 반환한다. 
그렇지 않으면 False가 반환된다 (예 : Arg1 <= Arg2).
...
첫 번째 인수가 두 번째 인수보다 크면 부울 true를 반환한다. 
그렇지 않으면 False가 반환된다 (예 : Arg1> Arg2).
...
첫 번째 인수가 두 번째 인수보다 크거나 같은 경우 부울 true를 반환한다. 
그렇지 않으면 False가 반환된다 (예 : Arg1> = Arg2).
...
- 사전형: `{}`
- 부울형: `false`
- `nil` 의 경우 항상 (null로 알려진)

구조체의 경우 비어있는 정의가 없으므로, 구조체는
기본 값을 반환하지 않는다.
...
서로 옆에있는 두 인수가 문자열이 아닌 경우 그 사이에 
공백이 추가된다.
...
`compact` 는 비어 있는 (즉, "") 항목이 제거 된 새 목록을 반환한다.
...
`isKind` 함수를 사용하면 값이 특정 종류인지 확인할 수 있다.
...
타입은 다루기가 약간 어렵기에, 세가지 다른 함수가 있다.
...
프리-릴리스 버전을 비교하지 않고 제약 조건을 사용하는 유의적 버전의 비교는 시험판 버전을 건너 뛴다. 
...
대소 문자 구분에서 기대할 수 있는 내용은 여기에 적용되지 않는다. 
이것은 사양이 지정하는 ASCII 정렬 순서 때문이다.
...
차트 내에서 비 특수 파일을 얻을 수 있는 몇 가지 기능이 있다. 
