...
기본 제공 헬름 코드 베이스의 일부는 아니다.

기존 플러그인은 
[연관]({{< ref "related.md#helm-plugins">}}) 섹션 또는 
...
헬름 플러그인은 헬름과 원활하게 통합되는 추가 도구이다. 
헬름은 핵심 기능 세트를 확장하는 방법을 제공하지만
모든 새로운 기능을 Go로 작성하고 핵심도구에 추가하지 않아도 된다.

헬름 플러그인에는 다음과 같은 기능들이 있다.

- 핵심 헬름 도구에 영향을 주지 않고 헬름 설치에서 추가 및
  제거 가능
- 모든 프로그래밍 언어로 작성 가능
- 헬름과 통합되며 `helm help` 및 기타 위치에 표시

헬름 플러그인은 `$HELM_PLUGINS` 에 있다. 
`helm env` 명령을 사용하여 환경에 설정되지 않은 경우 기본값을 포함하여 현재 값을 찾을 수 있다.

헬름 플러그인 모델은 부분적으로 Git의 플러그인 모델에서 모델링되었다.
이를 위해 사용자는 _plumbing_인 플러그인이 때때로 _porcelain_ 레이어라고 불리는 것을 들을수도 있다.
이것은 헬름이 사용자 경험과 최상위 수준의 처리 로직을 제공하는 반면 플러그인은 원하는 작업을 수행하는 "세부 작업"을 수행하도록 제안하는 간단한 방법이다.
...
위의 패턴은 플러그인 프로그램이있는 위치를 나타내기 위해 선호하는 방법을 보여준다.

플러그인 명령으로 작업하기 위한 몇 가지 전략이 있다.

- 플러그인에 실행 파일이 포함되어있는 경우 `platformCommand :` 또는 `command :`의 
  실행 파일이 플러그인 디렉토리에 패키징 필요.
- `platformCommand :`또는`command :` 행에는 
  실행 전에 확장 된 환경 변수가 있습니다. 
  `$ HELM_PLUGIN_DIR`은 플러그인 디렉토리를 지정.
- 명령 자체는 쉘에서 실행되지 않으며,
  따라서 사용자는 쉘 스크립트를 한 줄로 작성 불가.
- Helm이 환경 변수에 많은 구성을 삽입. 
  사용 가능한 정보를 확인하기 위해 환경 확인 필요
- Helm이 플러그인의 언어에 대해 가정하지 않으며,
  원하는대로 선정하여 작성 가능
- 명령어는`-h` 및`--help` 에 대한 특정 도움말 텍스트를 구현하는 역할을 수행.
...
다운로더 명령은 하위 명령 또는 인수도 지원하며, 예를 들어`plugin.yaml` 에 `bin / mydownloader 하위 명령 -d` 를 지정할 수 있다.
이는 기본 플러그인 명령과 다운로더 명령에 대해 동일한 실행 파일을 사용하지만 각각에 대해 다른 하위 명령을 사용하려는 경우에 유용하다.

## 환경 변수

Helm은 플러그인을 실행할 때, 외부 환경을 플러그인에 전달하고 추가 환경 변수도 삽입한다.

`KUBECONFIG`와 같은 변수는 외부 환경에서 설정 한 경우 플러그인에 대해 설정된다.
...
- `HELM_REGISTRY_CONFIG`: (사용하는 경우) 레지스트리 구성 위치. 
  레지스트리와 함께 헬름을 사용하는 것은 실험적인 기능임을 유의.
...
또한 Kubernetes 구성 파일이 명시 적으로 지정된 경우 'KUBECONFIG'변수로 설정된다.

## 플래그 구문 분석에 대한 유의 사항

플러그인을 실행할 때 Helm은 자체 사용을 위해 전역 플래그를 구문 분석한다. 
이러한 플래그는 플러그인에 전달되지 않는다.

- `--debug`: 지정하면`$ HELM_DEBUG`가`1`로 설정
- `--registry-config`: 이 플래그는 `$ HELM_REGISTRY_CONFIG` 로 변환
- `--repository-cache`: 이 플래그는 `$ HELM_REPOSITORY_CACHE` 로 변환
...
플러그인은 도움말 텍스트를 표시 한 다음 `-h` 및 `--help` 를 위해 종료_해야한다_. 
다른 모든 경우에는, 플러그인은 적절하게 플래그를 사용할 수 있다.

## 셸 자동-완성 제공

헬름 3.2부터, 플러그인은 Helm의 기존 자동 완성 메커니즘의 일부로 쉘 자동 완성에 대한 지원을 선택적으로 제공 할 수 있다.

### 정적 자동-완성

플러그인이 자체 플래그 및/또는 하위 명령을 제공하는 경우 플러그인의 루트 디렉토리에 `completion.yaml` 파일을 두어 헬름에 알릴 수 있습니다. 
...
제공되지 않는 경우 Helm은 플러그인(플러그인에서 [동적 완성] (#동적-완성)을 지원하지 않는 경우)에 대한 셸 자동 완성 기능을 제공하지 않는다.
...
또한 헬름 3.2부터 플러그인은 자체 동적 셸 자동 완성 기능을 제공 할 수 있다.
동적 셸 자동 완성은 미리 정의 할수 없는 매개 변수 값 또는 플래그 값의 완성이다.
...
기본 플러그인 실행 파일이 특별한 매개변수 또는 플래그를 감지하면 플러그인완료를 실행하는 것을 알게된다. 
...
1. 셸은 사용자 입력과 일치하지 않는 완성 선택 항목을 자동으로 필터링 한다. 
   따라서 플러그인은 사용자 입력과 일치하지 않는 완료를 제거하지 않고 관련된 모든 완료를 반환 할 수 있다.
   예를 들어, 명령 줄이 `helm fullstatus ngin <TAB>` 인 경우,
   `plugin.complete` 스크립트는 `ngin` 으로 시작하는 이름뿐 아니라 *모든* 릴리스 이름 (`default` 네임 스페이스의)을 출력 할 수 있다. 
   셸은 'ngin'으로 시작하는 것만을 유지한다.
1. 특별히 복잡한 플러그인이 있는 경우 동적 완성 지원을 단순화하기 위해 `plugin.complete` 스크립트가 기본 플러그인 스크립트를 호출하고 완료 선택을 요청하도록 할 수 있다. 
   예제는 위의 [동적 완성](#동적-완성) 섹션을 참조하자.
1. 동적 완성 및 `plugin.complete` 파일을 디버깅 하기 위해, 
   다음을 실행하여 완료 결과를 확인할 수 있따.
