...
차트는 디렉터리 안에 파일들의 모음으로 구성된다. 디렉터리의

이름은 (버전 정보 없는) 차트의 이름이다. 따라서,

 WordPress 를 설명하는 차트는 `wordpress/` 폴더에 저장된다.
...
헬름은 `charts/`, `crds/`, `templates/` 폴더와 나열된 파일 이름의 사용을
...
다르게, 헬름 v2 이상은 버전 번호를 release 마커로 사용한다.
...
의존하지도, 필요로 하지도 않는다. 따라서 깃 SHA를 버전관리에서는 모두
...
패키지 명 안의 버전 번호가 `Chart.yaml` 안의 버전 번호와
