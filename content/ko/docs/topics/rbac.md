...
쿠버네티스에서 사용자 또는 어플리케이션 별 서비스 계정에 
역할을 부여하는 것은 어플리케이션이 지정한 범위에서 작동하는지 
확인할 수 있는 모범 사례이다. [공식 쿠버네티스 
문서에서](https://kubernetes.io/docs/admin/authorization/rbac/#service-account-permissions) 서비스 계정 권한에 대하여 
자세히 알아보도록 하자

쿠버네티스 1.6 부터는 역할 기반 접근 제어가 기본적으로 활성화 된다.
RBAC를 사용하면 조직에서 사용자와 역할에 따라 허용되는
작업유형을 지정할 수 있다.

RBAC을 사용하여, 사용자는 다음을 수행할 수 있다.

- 관리자에게 권한이 있는 작업(새 역할과 같은 클러스터 전체 리소스 생성)을 
  부여한다.
- 리소스(파드, 퍼시스턴트 볼륨, 
  디플로이먼트)를 생성하는 사용자의 기능을 특정 네임스페이스 또는 클러스터 전체 범위(리소스 
...
모든 쿠버네티스 클러스터에는 2가지 사용자 카테고리가 있다: 쿠버네티스가 관리하는 서비스 계정과 
일반사용자

일반 사용자는 외부의 독립적인 서비스에 의해 관리되는 것으로 간주한다. 개인키를 
배포하는 관리자, 키스톤 또는 구글 계정과 같은 사용자 저장소, 심지어는 
사용자 이름 및 비밀번호 목록이 있는 파일까지. 이와 관련하여 쿠버네티스는 
일반 사용자 계정을 나타내는 객체를 가지고 있지 않다. 일반 사용자는 
...
시크릿을 볼수 있는 클러스터롤이 없다는 것을 눈치챘을 수도 있다. 
`view` 클러스터롤은 에스컬레이션 문제로 인해 사용자에게 시크릿에 
대한 읽기 접근 권한을 부여하지 않는다. 헬름은 기본적으로 실리스 메타 데이터를 시크릿으로 저장한다.
