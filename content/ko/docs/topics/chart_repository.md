...
이 섹션에서는 Helm 차트 리포지토리를 만들고 작업하는 방법을 설명한다.
고수준에서 차트 저장소는 패키지형 차트를 저장하고 공유할 수 있는 장소다.

공식 차트 저장소는 [쿠버네티스 차트](https://github.com/helm/charts)가 관리하고
있으며 참여를 환영한다. 하지만 헬름도 자신만의 차트 저장소를 쉽게 만들고 운영할
수 있게 해준다. 이 가이드는 그렇게 하는 방법을 설명한다.
...
_차트 저장소_ 는 `index.yaml` 파일과 패키지화된 차트를 저장하는 HTTP 서버다.
차트를 공유할 준비가 되면 차트 저장소에 업로드하는 것이 가장 선호되는 방법이다.
...
다음 단계는 예제 워크플로우를 구성하지만 차트 저장소에 차트를 저장하고 갱신하는
