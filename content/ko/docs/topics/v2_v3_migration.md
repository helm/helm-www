...
이 가이드에서는 헬름 v2를 v3로 마이그레이션 하는 방법을 보여준다.
헬름 v2가 설치되어 있어야 하며, 일대 다 클러스터에서 릴리스가 관리되어야 한다.
...
1. Tiller 제거:
   - 클라이언트/서버를 클라이언트/라이브러리 아키텍처로 대체 (`helm` 바이너리만
     해당)
   - 보안 서비스는 사용자 단위로 제공 (쿠버네티스 사용자 클러스터 보안에
     위임됨)
   - 릴리스는 클러스터 내 시크릿으로 저장되며 릴리스 개체 메타 
     데이터가 변경
   - 릴리스는 더 이상 Tiller 네임 스페이스가 아닌 릴리스 네임스페이스 
     기반으로 유지
2. 차트 저장소 업데이트:
...
1. 동일 클러스터를 관리하는 헬름 v2 및 v3:
   ...
   - 그렇지 않으면 다음과 같은 차이점으로 인해
	 ...
	 - 유일한 문제는 쿠버네티스 클러스터 범위의 
	   ...
	   이렇게 되면 리소스가 충돌하므로 네임스페이스에서 고유하더라도 v3 배포는 실패한다.
	 - v3 구성은 더이상 `$HELM_HOME` 을 사용하지 않고 대신 XDG 디렉토리 사양을 
	   사용한다. 또한 필요에 따라 즉시 생성된다. 따라서 v2 구성과 
	   무관하다. 두 버전이 동일한 시스템에 
	   설치된 경우에만 적용된다.
