...
it is called. The following rules will apply in deciding which command to use:
...
- If `platformCommand` is present, it will be searched first.
  ...
  - If `os` and `arch` are empty, the command will be used.
- If no `platformCommand` match is found and the deprecated `command` is present
  it will be used.
